...
db.collection('typing_scores')
    .orderBy('wpm', 'desc')
    .limit(20) // Mengambil 20 data teratas untuk memastikan kita mendapat 3 user yang berbeda
    .onSnapshot(snapshot => {
        topTypistsUIDs = [];
        snapshot.forEach(doc => {
            const uid = doc.data().uid;
            // Hanya masukkan UID jika belum ada di list, dan batasi maksimal 3 orang (Top 3)
...
window.sendFriendRequest = async function(targetUid) {
    ...
    try {
        ...
        if(searchUserInput) searchUserInput.dispatchEvent(new Event('input')); // Segarkan tombol pencarian agar jadi "Menunggu"
    } catch(err) {
...
window.acceptRequest = async function(senderUid) {
    ...
    try {
        ...
        // Tambah ke daftar teman masing-masing & hapus dari pending
        batch.update(myRef, {
...
window.openChat = function(friendUid, friendName) {
    ...
    document.getElementById('chatFriendName').innerHTML = `<i class="fas fa-comments"></i> ${friendName}`;
    ...
    unsubChatMessages = db.collection('chats').doc(currentChatId).collection('messages')
        ...
        .onSnapshot(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                div.innerHTML = `${msg.text} <div class="chat-time">${time}</div>`;
...
async function endGame() {
    ...
    if (finalWPM > 0) {
        ...
        try {
            await db.collection('typing_scores').add({
